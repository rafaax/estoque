...
function buscaRegistro($id){
    ...
    $sql = "SELECT * from compras where pagamento_id = $id";
    $query = mysqli_query($conexao, $sql);
    $count = mysqli_num_rows($query);
...
function logBanco($user, $id){
    $conexao = mysqli_connect('localhost', 'root', '', 'vetorsys');
    $sql = "INSERT INTO logs(usuario, mensagem, target) values ('$user','Delete Pagamento', '$id' )";
    ...
    if(buscaBanco($id)){
        ...
        if(buscaRegistro($id)){
            ...
            if($query){
             ...
             echo json_encode(array(
                'erro' => true,
                'msg' => 'Não pode ser deletado pois existem registros atrelados!'
            ));
            exit();
